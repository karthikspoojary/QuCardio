...
This design reveals an interaction effect rather than two additive gains. Under linear entanglement, the encoding range has no effect: both [0, 1] and [0, pi] produce 89.78%. The encoding range becomes consequential only once the entanglement topology is changed to circular, where it contributes a further 2.15 percentage points. Circular entanglement is therefore the necessary precondition for the encoding effect to appear.
...
On the circuit design side, a complete 2×2 factorial experiment over encoding range and entanglement topology reveals an interaction effect. Under linear entanglement, the encoding range has no effect at all: [0, 1] and [0, pi] both yield 89.78%, which is also the accuracy of the Prabhu et al. [1] base configuration. The entire 4.84-point gain over that configuration is attributable to closing the qubit chain into a ring; the encoding range then contributes a further 2.15 points, but only once circular entanglement is already in place. This interaction is a stronger finding than two additive effects: circular entanglement is the necessary precondition for the encoding range to matter. The same [0, pi] encoding range was independently adopted by Aksoy and Ozpolat [14] on unrelated medical datasets, supporting the interpretation that its advantage is a property of the ZZFeatureMap rotation structure rather than a dataset-specific artefact. The Pauli feature map ablation adds that the ZZFeatureMap outperforms the ZFeatureMap by 6.99 percentage points and the X + XX map by 2.68 percentage points, which establishes it as a justified choice rather than a default.
