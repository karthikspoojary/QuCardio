...
The principal finding from the ablation experiments is that the two most consequential circuit parameters, the feature encoding range and the entanglement topology, interact rather than contributing independently. Considering the encoding range first: with circular entanglement held fixed, scaling the nine-dimensional SVD-compressed features to [0, pi] rather than [0, 1] before ZZFeatureMap encoding raises QSVC accuracy from 92.47% to 94.62%, a gain of 2.15 percentage points (Table 7.2). Under linear entanglement the same change produces no gain at all, and this dependence between the two parameters is examined fully in Section 7.2. The mechanism lies in the rotation structure of the circuit: the ZZFeatureMap applies Rz rotations through an angle of 2·x_i radians about the Z axis, so a feature value confined to [0, 1] sweeps only about 2 radians of the available 2pi of azimuthal phase, whereas the [0, pi] range sweeps the full circle and separates the encoded states as widely as the circuit permits.

The second finding is the entanglement topology. Circular entanglement adds one entangling pair that closes the qubit chain, so that the two end qubits acquire a second neighbour and the interaction graph becomes a ring with no endpoints. With the encoding range held at [0, pi], this single additional pair raises accuracy by 4.84 percentage points over the linear topology, and with the range held at [0, 1] it raises accuracy by 2.69 percentage points (Table 7.3 and Section 7.2). This indicates that the correlation between the first and last SVD components carries discriminative information that an open chain cannot represent, and that the ring must be closed before a wider encoding range can be exploited.
...
Taken together, these contributions advance the state of the art in a specific direction. The prior quantum ECG classifiers report isolated accuracy figures; this project adds a systematic circuit design analysis over a 320-configuration sweep, a statistical test of the quantum advantage, calibration-verified confidence scores suitable for clinical decision support, and a working web interface, which moves quantum ECG classification from a proof of concept towards a validated and usable system.
...
- The History tab shall present a paginated view of the audit log of past predictions.
...
- Single prediction with a quantum model: end-to-end latency of under 900 ms (measured: 893.9 ms total for the QSVC — preprocessing 126.9 ms, ResNet50 feature extraction 627.6 ms, SVD projection 52.0 ms, quantum kernel classification 87.4 ms). The classification step itself is comparable to the classical SVM because the training statevectors are precomputed and inference requires only dense complex inner products with no circuit simulation at query time; the dominant cost is ResNet50 feature extraction, shared by all three models.
...
- The MobileNetV2 gatekeeper model is supplemented by a multi-criterion heuristic validator that checks aspect ratio, colour saturation, row and column coverage by dark pixels, column oscillation frequency, and high-frequency FFT energy. The heuristic is the active gate when the trained model is absent; when present, the ML model takes precedence. ECG prints with unusually large header or footer whitespace may be incorrectly rejected by the heuristic; the row-coverage threshold is set at 25% to accept all image formats observed across both datasets while rejecting screenshots and text documents. Digital ECG signal plots (such as PTB-XL renderings with axis labels and whitespace margins) may fail the coverage or oscillation checks and should be treated as out-of-scope for the current gatekeeper.
...
The dataset originates from the Ch. Pervaiz Elahi Institute of Cardiology, Multan, Pakistan, and is publicly available through Mendeley Data. It contains ECG paper-print images from clinical recordings across four cardiovascular classes. Each image is a scanned or photographed ECG record and exhibits the natural variation in scan quality, orientation and background that accompanies clinical collection. The standard four-class subset of this collection contains 929 images (Normal 284, Arrhythmia 233, Myocardial Infarction 240, History of MI 172), as reported by Prabhu et al. [1]. The count used here is 928: one Myocardial Infarction scan returned `None` from `cv2.imread` during feature extraction (see `src/features/extract_resnet_features.py`, line 32), indicating a corrupt or unreadable file, and was excluded at that stage. This accounts for the one-image discrepancy. The class counts were verified directly against the stored feature file `data/features_9d.npz`.
...
|---|---|---|---|---|
| AT-01 | FR-1, FR-6 | The upload flow can be completed and the class, confidence and recommendation read from the result view without consulting the source code or the API documentation | Task completed using the interface alone | PASS |
...
Acceptance testing was performed internally by the authors against the functional requirements.
...
The two ablation tables are part of a complete 2×2 factorial design, with the fourth cell — [0, 1] encoding with linear entanglement, the configuration of Prabhu et al. [1] — measured separately and yielding 89.78% (confirmed from `results/paper/ablation/b1_01_linear.json`). The full 2×2 is:
...
This design shows that the two factors interact rather than adding independently. Under linear entanglement the encoding range has no effect: both [0, 1] and [0, pi] produce 89.78%. Changing the topology alone, with the range held at [0, 1], is worth 2.69 percentage points. Changing both is worth 4.84 percentage points, which exceeds the sum of the two separate effects by 2.15 points; that excess is the interaction term. Circular entanglement is therefore the precondition for the encoding range to have any effect at all.

Two cautions apply to this reading. The four cells differ by between zero and nine test images and carry no variance estimate, so the magnitudes are indicative. Equal accuracy in the two linear cells also does not establish that the two configurations behave identically: they may misclassify different samples and arrive at the same total. Comparing the two prediction vectors directly, and applying McNemar's test between them, would settle whether the two configurations are genuinely equivalent. **[VERIFY: run this comparison. If the two prediction vectors turn out to be identical rather than merely equal in accuracy, the encoding change did not reach the circuit and the run needs to be repaired before this section can stand.]**

The [0, pi] range spans the full azimuthal circle available to the Rz(2·x_i) rotations, whereas [0, 1] uses barely a third of it, so the encoded states remain crowded together and the kernel values concentrate near their mean. Extending the range to [0, 2pi] gives up a little accuracy, because rotation angles beyond 2pi wrap around and distinct feature values begin to map to the same quantum state. L2 normalization performs worst, at 68.28%, because projecting onto the unit sphere discards the magnitude of the feature vector, and magnitude is what the uncentred SVD components encode. Closing the qubit chain into a ring, by contrast, adds a single entangling pair between the first and last qubits at a negligible increase in circuit depth, and the size of the resulting gain indicates that the correlation between those two SVD components carries discriminative information for this classification task.
...
Removing entanglement, as the ZFeatureMap does, produces the largest single drop in this comparison: it applies only single-qubit Rz rotations and therefore encodes each SVD component independently, falling to 87.63%, 6.99 percentage points below the ZZFeatureMap. Whatever joint morphological information the SVD components carry, an unentangled circuit cannot represent it.

The X + XX map reaches 91.94%, comfortably above the ZFeatureMap because it does entangle, but still 2.68 percentage points below the ZZFeatureMap. The Y + YY map falls furthest, to 83.33%, roughly level with the classical SVM baseline; the combination of Y-axis rotations with YY interactions appears to produce destructive interference in the kernel for this feature distribution, though this was not isolated further.
...
Under an increased brightness perturbation, which simulates an overexposed photograph of a printed ECG, the accuracy of the classical SVM falls from 84.4% to 46.7%, close to chance level on four classes, whereas the QSVC retains 86.7% under the same perturbation. This difference is indicative, having been measured on a targeted evaluation subset of 45 images held fixed across all perturbation conditions (38/45 = 84.4% baseline, 21/45 = 46.7% under brightness increase, 39/45 = 86.7% for the QSVC under the same condition; confirmed from augmentation_robustness.json in the repository).
...
On the circuit design side, a complete 2×2 factorial experiment over encoding range and entanglement topology shows that the two factors interact rather than adding independently. Under linear entanglement the encoding range has no effect at all, with [0, 1] and [0, pi] both yielding 89.78%. Closing the qubit chain into a ring raises accuracy to 92.47% under [0, 1] encoding, a gain of 2.69 percentage points, and to 94.62% once the range is widened to [0, pi]. The joint gain of 4.84 percentage points over the base configuration exceeds the sum of the two separate effects, 2.69 and 0.00, by 2.15 points, which is the interaction term of the design. Practically, this means circular entanglement has to be established first: only once the chain is closed does widening the encoding range produce any measurable gain. The same [0, pi] encoding range was independently adopted by Aksoy and Ozpolat [14] on unrelated medical datasets, supporting the interpretation that its advantage is a property of the ZZFeatureMap rotation structure rather than a dataset-specific artefact. The Pauli feature map ablation adds that the ZZFeatureMap outperforms the ZFeatureMap by 6.99 percentage points and the X + XX map by 2.68 percentage points, which establishes it as a justified choice rather than a default.
...
- **Seed variance.** All results come from a single stratified split at random\_state = 42 with no repeated runs. Repeating the main comparisons across five to ten seeds would attach variance estimates to the reported accuracy differences, which is likely to be required for journal submission.
- **Quanvolutional Neural Network architecture.** Implementing the Quanvolutional Neural Network of Prabhu et al. [1] using PennyLane quanvolutional filters would test whether a higher accuracy ceiling is reachable than the one attained by the kernel method.
...
- **Per-configuration KTA sweep.** The Kernel Target Alignment (KTA = 0.1081) has been computed for the optimal configuration only. Computing KTA across all 320 ablation configurations would provide a label-aware measure that complements the accuracy figures and may reveal whether alignment predicts the accuracy ordering.
...
[1] S. Prabhu, S. Gupta, G. M. Prabhu, A. V. Dhanuka, and K. V. Bhat, "QuCardio: Application of quantum machine learning for detection of cardiovascular diseases," *IEEE Access*, vol. 11, pp. 136122-136135, 2023, doi: 10.1109/ACCESS.2023.3338145.
...
[6] M. Y. Ramkhelawan, S. Grandhi, and S. Wibowo, "A hybrid quantum-classical deep learning algorithm for efficient arrhythmia classification," in *Proc. IEEE/ACIS Int. Conf. Softw. Eng., Artif. Intell., Netw. Parallel/Distrib. Comput. (SNPD)*, 2025, pp. *(confirm on IEEE Xplore)*, doi: 10.1109/SNPD62189.2025.10985682.
...
[9] H. S. B. Gummadi, S. K. Thota, S. Akuthota, M. Guduri, S. D. Veeravalli, and H. Vemuganti, "Quantum-based convolutional neural network model for efficient cardiovascular disease prediction," in *Proc. 3rd IEEE Int. Conf. Artif. Intell. (ThingsAI)*, 2025, pp. *(confirm on IEEE Xplore)*, doi: 10.1109/ThingsAI65156.2025.10926651.
...
[11] F. Vasquez-Iturralde, M. J. Flores-Calero, F. Grijalva, and A. Rosales-Acosta, "Automatic classification of cardiac arrhythmias using deep learning techniques: A systematic review," *IEEE Access*, vol. 12, pp. 118467-118492, 2024, doi: 10.1109/ACCESS.2024.3408282.
...
[18] World Health Organization, "Cardiovascular diseases (CVDs)," WHO Fact Sheet, Geneva, Switzerland. [Online]. Available: https://www.who.int/news-room/fact-sheets/detail/cardiovascular-diseases-(cvds) [Accessed: 18 Sep. 2026].

[19] India State-Level Disease Burden Initiative CVD Collaborators, "The changing patterns of cardiovascular diseases and their risk factors in the states of India: the Global Burden of Disease Study 1990–2016," *The Lancet Global Health*, vol. 6, no. 12, pp. e1339–e1351, Dec. 2018, doi: 10.1016/S2214-109X(18)30407-8.
...
3. Select one of the three classifiers. The QSVC gives the highest accuracy; measured end-to-end latency is under 900 ms (893.9 ms: preprocessing 126.9 ms, ResNet50 627.6 ms, SVD 52.0 ms, classification 87.4 ms). Because the training statevectors are precomputed, the quantum classification step (87.4 ms) is comparable to the classical SVM — the dominant cost is ResNet50 feature extraction, shared by all three models. A progress indicator reports the processing stage for any model.
...
| Long wait on a quantum model | ResNet50 feature extraction (≈628 ms) is the dominant cost, shared by all three models; the quantum kernel classification step (≈87 ms) is not the bottleneck | Total response is under 900 ms; wait for the progress indicator to complete |
