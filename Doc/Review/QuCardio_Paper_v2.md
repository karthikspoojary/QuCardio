...
This paper is therefore positioned as a circuit-design and validation study rather than as a claim of improved state-of-the-art accuracy. The final accuracy obtained here, 94.62%, is within one test image of the 94.09% reported by Prabhu et al. [5] on the same data, and no conclusion is drawn from that difference. The contribution lies in establishing which circuit design choices produce the accuracy, and in subjecting the result to tests that the prior literature omits. Specifically:

1. A controlled ablation over the encoding range, the entanglement topology, the Pauli feature map family, the circuit repetitions and the number of retained components, reported as a factorial design in which the contribution of the encoding range and the contribution of the topology are separated rather than confounded.
...
Prabhu et al. [5] demonstrated quantum ECG classification on the same four-class clinical data used here, combining ResNet50 pool1\_pool features with Truncated SVD at nine components and a ZZFeatureMap QSVC under [0, 1] feature scaling with linear entanglement. They reported 94.09% for the QSVC, 93.05% for a Multiclass Pegasos QSVC and 97.31% for an accompanying Quanvolutional Neural Network. Neither the encoding range nor the entanglement topology was varied, and no statistical validation, calibration analysis or deployment was described. The present work retains that architecture and adds the circuit design study.
...
|---|---|---|---|---|---|---|---|
| Prabhu et al. [5] | 4-class ECG images | ResNet50+SVD+QSVC ([0,1], linear) | 94.09% | No | No | No | No |
...
The experiments use the ECG image collection released on Mendeley Data by Khan et al. [16], collected at the Ch. Pervaiz Elahi Institute of Cardiology, Multan, Pakistan. The full collection contains 1937 paper ECG prints across categories that include COVID-19 cases outside the scope of this study. The standard four-class subset of this collection (Normal 284, Arrhythmia 233, Myocardial Infarction 240, History of MI 172) yields 929 images as reported in [5]. The experiments here use 928 images; one Myocardial Infarction scan returned `None` from `cv2.imread` during preprocessing — consistent with a corrupt or unreadable file — and was silently dropped at the feature-extraction stage (`src/features/extract_resnet_features.py`, line 32). This is the complete explanation for the one-image discrepancy. Table II shows the resulting class distribution and the stratified 80:20 split applied with random\_state = 42. No augmentation was applied during training; augmented images appear only in the robustness evaluation of Section V-D.
...
Kernel Target Alignment (KTA) [18] was computed for the optimal QSVC configuration (ZZFeatureMap, [0, pi] encoding, circular entanglement, reps = 2), giving 0.1081. The value is confirmed from kta_score.json in the repository (exact value: 0.10808514884073321). The value is positive, indicating agreement in sign between the kernel matrix and the label structure, but it is reported as a descriptive statistic only: without a matched KTA value for the classical RBF kernel on the same features, the magnitude alone does not establish that the quantum kernel is better aligned.
...
The tuned QSVC leads the four other classifiers, with a margin of 9.67 percentage points over the classical SVM. The classical SVM exceeds the corresponding baseline in [5] by 1.62 points, which we tentatively attribute to balanced class weighting and to the resampling performed by the calibration wrapper, although this was not ablated. Pegasos falls 1.11 points below the 93.05% reported in [5]; the likely cause is stricter leakage control here, since the SVD reducer and the scaler are fitted on training data only and the corresponding detail is not fully specified in the prior work. Random Forest reaching 91.94% without any quantum component is informative on its own: the nine-dimensional SVD features already carry strong discriminative signal, so the quantum kernel is improving on an informative representation rather than compensating for a weak one.
...
That internal framing is necessary for a second reason. Re-running the configuration described in [5] on the present pipeline does not reproduce the accuracy reported there. Because the exact split and random seed used in that work are not published, the discrepancy cannot be resolved from the paper alone, and we make no claim about its cause. All ablation gains in Section VI are therefore measured against configurations run on this pipeline, which keeps the circuit design comparisons internally consistent regardless of the reproduction gap. Readers should treat the absolute accuracies here as pipeline-specific and the differences between configurations as the result.
...
Fig. 5 shows model accuracy under nine augmentation conditions, evaluated on a subset of 45 test images held fixed across all conditions (38/45 = 84.4%, 39/45 = 86.7%, 21/45 = 46.7%). Under brightness perturbation, which simulates an overexposed photograph of a paper ECG print, classical SVM accuracy falls from 84.4% to 46.7%, close to chance on four classes, while the QSVC holds 86.7%. Given the subset size, the difference is reported as indicative rather than as a precisely estimated effect.
...
*Note: all rows in this table use circular entanglement. The 92.47% figure for [0,1] encoding therefore corresponds to [0,1]+circular, not to the [0,1]+linear configuration used in [5]. The missing [0,1]+linear cell appears in Table VII-A.*
...
L2 normalisation is the clear outlier, 26.34 points below [0, pi] and 24.19 points below [0, 1]. Projecting onto the unit sphere discards feature magnitude entirely, and magnitude is precisely what the dominant uncentred SVD components encode.
...
*All four cells are confirmed from the repository. [0,1]+linear measured by run_b1.py (b1_01_linear.json). [0,pi]+linear from ablation_entanglement.json. [0,1]+circular from ablation_encoding.json. [0,pi]+circular from metrics_report.json.*

With the topology held circular, moving the encoding range from [0, 1] to [0, pi] gains 2.15 percentage points. With the encoding range held at [0, pi], moving the topology from linear to circular gains 4.84 percentage points. The topology therefore carries the larger share of the improvement once the encoding range is no longer the binding constraint.

The complete factorial design reveals that the two factors interact rather than adding independently. The [0, 1] and [0, pi] encodings produce identical accuracy (89.78%) under linear entanglement, so the encoding range contributes nothing until the qubit chain is closed into a ring. The full gain of 4.84 percentage points over the Prabhu et al. [5] configuration is attributable entirely to the topology change; the encoding range adds a further 2.15 points only once circular entanglement is already in place. This interaction is a stronger result than two additive effects: it identifies circular entanglement as the necessary precondition for the encoding range to matter, and it implies that studies which fix linear entanglement will not observe the encoding effect even if they vary the range.
...
The ZFeatureMap applies only single-qubit Rz rotations with no entanglement and reaches 87.63%, which is 6.99 percentage points below the ZZFeatureMap. That gap quantifies the contribution of the ZZ entangling interactions directly: removing entanglement while holding everything else fixed costs 13 test samples. The X+XX variant adds entanglement through a different gate basis and reaches 91.94%, above the unentangled result but 2.68 points short of the ZZFeatureMap. The Y+YY variant is the outlier at 83.33%, below both the unentangled map and the classical SVM baseline; the combination of Y-axis rotations with YY interactions appears to interfere destructively in the kernel for this feature distribution, but the mechanism was not isolated and no claim is made beyond the empirical observation on a single dataset.
...
Five constraints bound the conclusions of this study.

All results come from a single dataset collected at one institution and from a single stratified split at random\_state = 42. No repeated runs across seeds were performed, so no variance estimates accompany the reported accuracies. Differences below roughly one percentage point, corresponding to one or two test images, should be treated as noise. The ablation effects reported in Section VI are larger than this threshold, but their precision is unquantified, and repeating the sweep across several seeds is the first item of further work.

The configuration reported in [5] was not reproduced on the present pipeline, and the cause could not be determined because the split and seed used there are not published. Absolute accuracies in this paper are therefore pipeline-specific, and only the differences between configurations evaluated here should be compared across papers.

Generalisation across scanner environments is limited. In a zero-shot transfer to a second Mendeley ECG dataset from the same clinical source, QSVC accuracy fell to 34.09% on a three-class evaluation while the classical SVM degraded more gradually to 62.66%, with the QSVC collapsing most predictions into a single class. The quantum kernel appears to learn a precise but scanner-specific decision boundary, so the advantage measured in Sections V and VI is established for the training domain and not beyond it.

All quantum results come from ideal statevector simulation. No noise model was applied and no hardware execution was attempted, so the reported accuracies are an upper bound on what a NISQ device would deliver at nine qubits.

The extraction layer was adopted from [5] rather than selected empirically. No layer-wise ablation comparing pool1\_pool against deeper ResNet50 outputs was run, so the claim that shallow morphological features suit this task rests on the prior work rather than on evidence presented here.

---

## VIII. CONCLUSION

This paper reported a controlled study of how quantum circuit design choices affect QSVC accuracy for four-class ECG image classification, on a 928-image clinical subset of the Khan et al. [16] ECG collection. A complete 2x2 factorial design over the encoding range and entanglement topology reveals an interaction effect: under linear entanglement, [0, 1] and [0, pi] encoding produce identical accuracy (89.78%), so the encoding range contributes nothing until the chain is closed into a ring. Switching to circular entanglement raises accuracy to 92.47% under [0, 1] and to 94.62% under [0, pi], a further 2.15 points. Circular entanglement is therefore the necessary precondition for the encoding effect to appear. The Pauli map comparison shows that the ZZ interactions specifically, and not entanglement in general, produce the result, since the X+XX and Y+YY variants both fall short.
...
The final accuracy of 94.62% is within one test image of the figure reported in [5] on the same data, and the paper makes no claim of improved state-of-the-art accuracy. Its contribution is to establish which circuit choices produce that accuracy and to subject the result to significance, calibration, robustness and transfer tests that the prior literature omits. The transfer test is the sharpest of these: the advantage does not survive a change of scanner, which is the result that most constrains any clinical reading of the rest.
...
The ECG image dataset used in this study is publicly available on Mendeley Data [16]. The implementation, trained model artefacts and the scripts reproducing all tables and figures are available at https://github.com/yourusername/QuCardio.
...
All items below have been verified and reconciled in QuCardio_Report_v4.md.

| Item | Paper value | Report status |
|---|---|---|
| Sweep size | 320 configurations | ✓ Fixed — report correctly states 320 |
| Prabhu's fourth model | Quanvolutional Neural Network (QNN) | ✓ Fixed — report already uses "Quanvolutional" |
| Reference [5] pages | 136122–136135 | ✓ Fixed — report already uses 136122–136135 |
| Vasquez-Iturralde pages | 118467–118492 | ✓ Fixed — report already uses 118467–118492 |
| Augmentation subset size | n = 45 (confirmed from JSON) | ✓ Fixed — report now states n = 45 with exact fractions |
| Quantum inference latency | Same order as classical | ✓ Fixed — Appendix II no longer says "takes longest" |
| 89.78% cell | [0, pi] + linear (from ablation_entanglement.json) | ✓ Consistent — both documents attribute correctly |
| B2 (RF/KNN precision/recall) | RF: 91.39%/91.80%, KNN: 85.54%/85.07% | ✓ Filled in Table III from classical_baselines.json |
| B3 (augmentation n) | n = 45 | ✓ Filled — 38/45 = 84.4%, 21/45 = 46.7%, 39/45 = 86.7% |
| B5 (KTA, ECE) | KTA = 0.1081 (exact: 0.10808...), ECE 0.054/0.063/0.067 | ✓ Confirmed from kta_score.json; B5 tags removed |

**Still open (user input only — no more experiments needed):**
- ~~B1: CLOSED~~ — [0,1]+linear = 89.78% (run_b1.py, result: interaction effect confirmed).
- B4: Confirm the "<15% image inputs" and "<8% calibration" figures appear in Vasquez-Iturralde et al. [2] before restoring them to Section II.
- B6: Add the GitHub/Zenodo repository URL before submission.
